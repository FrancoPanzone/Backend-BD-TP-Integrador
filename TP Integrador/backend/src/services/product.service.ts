...
//   async getAll(): Promise<Product[]> {
//     return ProductRepository.getAll(); // no transaction
//   }

//   // Obtener producto por ID
//   async getById(id: number): Promise<Product | null> {
//     return ProductRepository.getById(id); // no transaction
//   }

//   // Crear producto (no atómico, no necesita transacción)
//   async create(data: ProductInput): Promise<Product> {
//     // Validar categoría
//     const category = await CategoryService.getById(data.category_id);
...
//   // Actualizar producto (no atómico, no necesita transacción)
//   async update(id: number, data: Partial<ProductInput>): Promise<Product | null> {
//     if (data.category_id) {
//       const category = await CategoryService.getById(data.category_id);
//       if (!category) throw new Error(`La categoría con id ${data.category_id} no existe`);
...
//   // ↓ Métodos que sí necesitan transacción (para órdenes u operaciones atómicas) ↓
...
//   //   const updated = await ProductRepository.update(productId, { stock: product.stock - quantity }, transaction || null);
//   //   if (!updated) throw new Error('Error al actualizar el stock');
//   //   return updated;
//   // }
...
//   //   return updated;
//   // }
//   async decreaseStock(productId: number, quantity: number, transaction: Transaction | null = null): Promise<Product> {
//     const product = await this.getById(productId);
//     if (!product) throw new Error('Producto no encontrado');
//     if (product.stock < quantity) throw new Error('Stock insuficiente');
...
//   async increaseStock(productId: number, quantity: number, transaction: Transaction | null = null): Promise<Product> {
//     const product = await this.getById(productId);
//     if (!product) throw new Error('Producto no encontrado');
