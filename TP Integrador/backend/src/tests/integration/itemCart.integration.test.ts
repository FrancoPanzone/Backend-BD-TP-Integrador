...
describe('ItemCart Integration Tests with Transactions', () => {
    ...
    beforeEach(async () => {
        ...
        const product = await ProductService.create(
            { name: 'Producto Test', description: 'Desc', price: 100, stock: 10, rating: 4, brand: 'Test', image: '/img/test.webp', category_id: 1 },
            transaction
        );
    ...
    it('should delete item from cart', async () => {
        const item = await itemCartService.create({ cart_id: cartId, product_id: productId, quantity: 1 }, transaction);
    ...
    it('should clear all items from cart', async () => {
        await itemCartService.create({ cart_id: cartId, product_id: productId, quantity: 1 }, transaction);
        await itemCartService.create({ cart_id: cartId, product_id: productId, quantity: 2 }, transaction);
